...
def model(fname,fpfn):
    ...
    train_x = np.stack([convert_to_input(fpfn,x_entry) for x_entry in train_x_entries])
    train_weights = np.stack([weight(x_entry) for x_entry in train_x_entries])
    train_y = np.stack(train_y)
    decomposer = sklearn.decomposition.TruncatedSVD(n_components=5000).fit(train_y)

    eval_x = np.stack([convert_to_input(fpfn,x_entry) for x_entry in eval_x_entries])
    eval_y = np.stack(eval_y)

    print("Training model.")
    model = sklearn.kernel_ridge.KernelRidge(kernel='linear').fit(train_x, decomposer.transform(train_y),sample_weight=train_weights)

    train_pred = decomposer.inverse_transform(model.predict(train_x))
    eval_pred = decomposer.inverse_transform(model.predict(eval_x))
    print("Eval",mrrmse.vectorized(torch.tensor(eval_pred),torch.tensor(eval_y)))

    test_x_entries = data.get_test()
    test_x = np.stack([convert_to_input(fpfn,x_entry) for x_entry in test_x_entries])
